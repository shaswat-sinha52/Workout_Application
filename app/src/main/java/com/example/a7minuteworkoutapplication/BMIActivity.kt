...
import com.example.a7minuteworkoutapplication.databinding.ActivityBmiactivityBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding?.btnCalculateUnits?.setOnClickListener {
            if(validatemetricunits()){
                val heigthvalue : Float=binding?.etMetricUnitHeight?.text.toString().toFloat()/100

                val weigthvalue : Float=binding?.etMetricUnitWeight?.text.toString().toFloat()

                val bmi=weigthvalue/(heigthvalue*heigthvalue)

                displayBMIResult(bmi)
            }
            else{
                Toast.makeText(this, "ENTRE DETAILS", Toast.LENGTH_SHORT).show()
